...
parser.add_argument('--root_dir', default='/home0/students/master/2024/liuyj/learn/Dataset/DUO/', type=str,
...
parser.add_argument('--random_split', action='store_true',
                    help="random split the dataset, default ratio is 6:2:2")
...
def train_test_val_split_random(img_paths, ratio_train=0.8, ratio_test=0.1, ratio_val=0.1):
    ...
    train_img, middle_img = train_test_split(img_paths, test_site=1 - ratio_train, random_state=233)
    ratio = ratio_val / (1 - ratio_train)
    val_img, test_img = train_test_split(middle_img, test_site=ratio, random_state=233)
...
def yolo2coco(arg):
    ...
    if arg.random_split or arg.split_by_file:
        ...
        for i, cls in enumerate(classes, 0):
            train_dataset['categories'].append({'id': i + 1, 'name': cls, 'supercategory': 'mark'})
            val_dataset['categories'].append({'id': i + 1, 'name': cls, 'supercategory': 'mark'})
            test_dataset['categories'].append({'id': i + 1, 'name': cls, 'supercategory': 'mark'})

        if arg.random_split:
            print("spliting mode: random split")
            train_img, val_img, test_img = train_test_val_split_random(indexes, 0.6, 0.2, 0.2)
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        txtFile = index.replace('images', 'txt').replace('.jpg', '.txt').replace('.jpeg', '.txt').replace('.png',
                                                                                                          '.txt')
        imageID = int(txtFile.split('.')[0])
        # 读取图像的宽和高
        im = cv2.imread(os.path.join(root_path, 'images/test/') + index)
