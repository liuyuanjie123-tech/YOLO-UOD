...
def nwd(box1, box2, scale1=0.5, eps=1e-7):  # 调整scale1的大小为0.5，1.0，1.5
...
#
#     @classmethod
#     def _scaled_loss(cls, self, gamma=1.9, delta=3):       # 调整参数 原始gamma=1.9,delta=3
...
class ConfusionMatrix:
    ...
    def plot(self, normalize=True, save_dir="", names=(), on_plot=None):
        ...
        seaborn.set_theme(font_scale=3 if nc < 50 else 1.0)  # for label size
        ...
        ax.set_xlabel("True", fontsize=17)
        ax.set_ylabel("Predicted", fontsize=17)

        # 放大刻度标签字体并旋转横坐标标签
        ax.tick_params(axis='both', which='major', labelsize=16)  # 整体刻度标签字体大小（可微调）

        # 单独设置x轴刻度标签旋转和对齐
        plt.xticks(rotation=45, ha='right', rotation_mode='anchor')  # 旋转45度并右对齐
        plt.yticks(fontsize=16)  # y轴刻度标签字体大小（与x轴保持一致）
...
class DetMetrics(SimpleClass):
    ...
    def keys(self):
        ...
        return ["metrics/precision(B)", "metrics/recall(B)", "metrics/mAP50(B)", 'metrics/mAP75(B)',
                "metrics/mAP50-95(B)"]
